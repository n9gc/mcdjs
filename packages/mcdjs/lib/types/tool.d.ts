...
/**把联合类型 {@link N} 拆分成元组 */
export type EachOfUnion<N, R extends any[] = []> = ({
    s: OneOfUnion<N>;
} extends Shred<{
    s: infer K;
}, {
    s: never;
}> ? EachOfUnion<Exclude<N, K>, [K, ...R]> : R);
/**由元组 {@link L} 中元素构成的不重复数组 */
export type UniqueItems<L extends any[], R extends any[] = []> = (L extends [infer S, ...infer K] ? [S, ...UniqueItems<[...R, ...K]>] | UniqueItems<K, [S, ...R]> : []);
/**以 {@link F} 为分隔符，将元组 {@link S} 拼接起来 */
export type Joined<S extends Tostrable[], F extends Tostrable, R extends string = ''> = (S extends [infer I extends Tostrable, ...infer S extends [any, ...any[]]] ? Joined<S, F, `${R}${I}${F}`> : `${R}${S[0]}`);
/**扔掉首元素 */
export type Shifted<T extends AnyArr> = T extends readonly [any, ...infer T] ? T : T;
/**一个键为 {@link C} 的除了其中键 {@link N} 以外全为可选 {@link T} 的对象 */
export type SigreqObj<N extends C, C extends string, T> = {
    [I in N]: T;
} & {
    [I in C]?: T;
};
/**一个存储数组 {@link T} 中元素和索引对应关系的对象 */
export type MapOfArray<T extends AnyArr> = {
    [I in Instr<BInT<keyof T, `${number}`>, number>]: T[I];
};
/**可以被反转的对象 */
export type Revable = {
    [x: number | string]: number | string;
};
type RevedObjOri<T extends Revable, N extends keyof T = BInT<keyof T, number | string>> = N extends N ? {
    [I in T[N]]: N;
} : never;
/**反转对象。就是把值作键，把键作值 */
export type RevedObj<T extends Revable> = InterOfUnion<RevedObjOri<T>>;
export declare function revMap<T extends Revable>(map: T): InterOfUnion<RevedObjOri<T, BInT<keyof T, string | number>>>;
/**把对象 {@link O} 中由 {@link T} 变成的字符串全变回去 */
export type EvaledMap<O, T extends Tostrable = NotStr> = {
    [I in keyof O]: O[I] extends `${Exted<infer K, T>}` ? K : O[I];
};
type SWLArr<L extends 0 | 1, A, S extends AnyArr> = [readonly [A, ...S], readonly [...S, A]][L];
/**切割元组的有限和无限部分 */
export type SWArray<S extends AnyArr, L extends 0 | 1 = 0, R extends AnyArr = []> = (S extends SWLArr<L, infer A, infer S> ? SWArray<S, L, [[...R, A], [A, ...R]][L]> : (L extends 0 ? [R, ...SWArray<S, 1>] : [S, R]));
/**更方便的匹配切割后的元组 */
export type SWTmpl<A extends [AnyArr, AnyArr, AnyArr]> = A;
type RevedArrLtd<T extends AnyArr, R extends AnyArr = []> = (T extends readonly [infer S, ...infer T] ? RevedArrLtd<T, [S, ...R]> : R);
/**反转数组 */
